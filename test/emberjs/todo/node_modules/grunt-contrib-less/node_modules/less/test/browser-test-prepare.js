...
var createTestRunnerPage = function(dir, exclude, testSuiteName, dir2) {
	...
	fs.readdirSync(path.join("test", dir, 'less', dir2 || "")).forEach(function(file) {
		...
		var name = path.basename(file, '.less'), id = (dir ? dir + '-' : "") + 'less-' + (dir2 ? dir2 + "-" : "") +
													  name;
		...
		output +=
		'<link id="original-less:' + id + '" rel="stylesheet/less" type="text/css" href="http://localhost:8081/' +
		path.join(dir, 'less', dir2 || "", name) + '.less' + '">\n';
		output += '<link id="expected-less:' + id + '" rel="stylesheet"  type="text/css" href="http://localhost:8081/' +
				  path.join(dir, 'css', dir2 || "", name) + '.css' + '">\n';
	});

	output +=
	String(fs.readFileSync(path.join('test/browser', 'template.htm'))).replace("{runner-name}", testSuiteName);
