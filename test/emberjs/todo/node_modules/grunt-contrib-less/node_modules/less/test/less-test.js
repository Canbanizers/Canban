var path = require('path'), fs = require('fs'), sys = require('util');
...
var totalTests = 0, failedTests = 0, passedTests = 0;
...
runTestSet(null, "errors/", function(name, err, compiledLess, doReplacements) {
	...
	return input.replace("{path}", path.join(process.cwd(), "/test/less/errors/")).replace("{pathrel}",
			path.join("test", "less", "errors/")).replace(/\r\n/g, '\n');
});

runTestSet({dumpLineNumbers: 'comments'}, "debug/", null, function(name) {
		return name + '-comments';
	});
runTestSet({dumpLineNumbers: 'mediaquery'}, "debug/", null, function(name) {
		return name + '-mediaquery';
	});
runTestSet({dumpLineNumbers: 'all'}, "debug/", null, function(name) {
		return name + '-all';
	});
runTestSet({relativeUrls: false, rootpath: "folder (1)/"}, "static-urls/");

function globalReplacements(input, directory) {
	var p = path.join(process.cwd(), directory), pathimport = path.join(process.cwd(),
			directory + "import/"), pathesc = p.replace(/[.:\/]/g,
			'\\$&'), pathimportesc = pathimport.replace(/[.:\/]/g, '\\$&');

	return input.replace(/\{path\}/g, p).replace(/\{pathesc\}/g, pathesc).replace(/\{pathimport\}/g,
			pathimport).replace(/\{pathimportesc\}/g, pathimportesc).replace(/\r\n/g, '\n');
}
