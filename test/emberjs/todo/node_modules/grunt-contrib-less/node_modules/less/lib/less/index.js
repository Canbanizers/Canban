var path = require('path'), sys = require('util'), url = require('url'), http = require('http'), fs = require('fs');
	...
	formatError: function(ctx, options) {
		...
		if (extract[1]) {
			error.push(ctx.line + ' ' + extract[1].slice(0, ctx.column) +
					   stylize(stylize(stylize(extract[1][ctx.column], 'bold') + extract[1].slice(ctx.column + 1),
						   'red'), 'inverse'));
		}
		...
		ctx.filename &&
		(message += stylize(' in ', 'red') + ctx.filename + stylize(':' + ctx.line + ':' + ctx.column, 'grey'));
...
['color', 'directive', 'operation', 'dimension', 'keyword', 'variable', 'ruleset', 'element', 'selector', 'quoted',
 'expression', 'rule', 'call', 'url', 'alpha', 'import', 'mixin', 'comment', 'anonymous', 'value', 'javascript',
 'assignment', 'condition', 'paren', 'media', 'ratio', 'unicode-descriptor'
].forEach(function(n) {
		require('./tree/' + n);
	});
...
less.Parser.importer = function(file, paths, callback, env) {
	...
	function parseFile(e, data) {
		...
		var rootpath = env.rootpath, j = file.lastIndexOf('/');
	...
	if (isUrl || isUrlRe.test(paths[0])) {
		...
		var urlStr = isUrl ? file : url.resolve(paths[0], file), urlObj = url.parse(urlStr), req = {
