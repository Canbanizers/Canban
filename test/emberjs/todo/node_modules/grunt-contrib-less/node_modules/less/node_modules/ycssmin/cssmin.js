...
var _compressHexColors = function(css) {
	...
	while ((m = pattern.exec(css)) !== null) {
		...
		if (isFilter) {
			...
			if (m[2].toLowerCase() === m[3].toLowerCase() && m[4].toLowerCase() === m[5].toLowerCase() &&
...
var _extractDataUrls = function(css, preservedTokens) {
	...
	var maxIndex = css.length - 1, appendIndex = 0, startIndex, endIndex, terminator, foundTerminator, sb = [
		], m, preserver, token, pattern = /url\(\s*(["']?)data\:/g;
...
function cssmin(css, linebreakpos) {
	...
	var startIndex = 0, endIndex = 0, i = 0, max = 0, preservedTokens = [], comments = [
		], token = '', totallen = css.length, placeholder = '';
	...
	while ((startIndex = css.indexOf("/*", startIndex)) >= 0) {
		...
		comments.push(token);
		css =
		css.slice(0, startIndex + 2) + "___YUICSSMIN_PRESERVE_CANDIDATE_COMMENT_" + (comments.length - 1) + "___" +
		css.slice(endIndex);
		startIndex += 2;
	}
		...
		if (token.length === 0) {
			...
			if (startIndex > 2) {
				if (css.charAt(startIndex - 3) === '>') {
					preservedTokens.push("");
					css =
					css.replace(placeholder, "___YUICSSMIN_PRESERVED_TOKEN_" + (preservedTokens.length - 1) + "___");
				}
			}
		}
	...
	css =
	css.replace(/(background-position|transform-origin|webkit-transform-origin|moz-transform-origin|o-transform-origin|ms-transform-origin):0(;|\})/gi,
