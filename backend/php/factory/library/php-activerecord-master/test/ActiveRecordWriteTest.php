...
class ActiveRecordWriteTest extends DatabaseTest {
	...
	public function test_insert_should_quote_keys() {
		$author = new Author(array('name' => 'Blah Blah'));
		$author->save();
		$this->assert_true(strpos($author->connection()->last_query,
				$author->connection()->quote_name('updated_at')) !== false);
	}
	...
	public function test_update_attributes_undefined_property() {
		$book = Book::find(1);
		$book->update_attributes(array('name'                      => 'new name', 'invalid_attribute' => true,
	...
	public function test_delete_all_with_limit_and_order() {
		...
		$num_affected = Author::delete_all(array('conditions' => array('parent_author_id = ?', 2), 'limit' => 1,
												 'order'      => 'name asc'));
	...
	public function test_update_all_with_conditions_as_hash() {
		$num_affected =
			Author::update_all(array('set' => 'parent_author_id = 2', 'conditions' => array('name' => "Tito")));
		$this->assert_equals(1, $num_affected);
	}
