...
class SerializationTest extends DatabaseTest {
	...
	public function test_only_should_only_apply_to_attributes() {
		...
		$this->assert_has_keys('book_id', 'upper_name',
			$this->_a(array('only' => 'book_id', 'methods' => 'upper_name')));
	}
	...
	public function test_include_nested_with_nested_options() {
		$a = $this->_a(
			array('include' => array('events' => array('except'  => 'title',
													   'include' => array('host' => array('only' => 'id'))))),
			Host::find(4));
	...
	public function test_only_method() {
		$this->assert_contains('<sharks>lasers</sharks>',
			Author::first()->to_xml(array('only_method' => 'return_something')));
	}
