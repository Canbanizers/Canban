...
class RelationshipTest extends DatabaseTest {
	...
	public function set_up($connection_name = null) {
		...
		Venue::$has_many = array(array('events', 'order' => 'id asc'),
			array('hosts', 'through' => 'events', 'order' => 'hosts.id asc'));
		Venue::$has_one = array();
	...
	public function test_belongs_to_with_explicit_foreign_key() {
		$old = Book::$belongs_to;
		Book::$belongs_to =
			array(array('explicit_author', 'class_name' => 'Author', 'foreign_key' => 'secondary_author_id'));
	...
	public function test_belongs_to_can_be_self_referential() {
		Author::$belongs_to =
			array(array('parent_author', 'class_name' => 'Author', 'foreign_key' => 'parent_author_id'));
	...
	public function test_has_many_with_sql_clause_options() {
		Venue::$has_many[0] = array('events',
			...
			'offset' => 1);
		Venue::first()->events;
		$this->assert_sql_has($this->conn->limit("SELECT type FROM events WHERE venue_id=? GROUP BY type", 1, 2),
	...
	public function test_has_many_through_with_conditions() {
		Event::$belongs_to = array(array('host'));
		Venue::$has_many[1] =
			array('hosts', 'through' => 'events', 'conditions' => array('events.title != ?', 'Love Overboard'));
	...
	public function test_has_many_with_explicit_keys() {
		$old = Author::$has_many;
		Author::$has_many = array(array('explicit_books', 'class_name'  => 'Book', 'primary_key' => 'parent_author_id',
	...
	public function test_has_one_with_joins() {
		...
		$this->assert_sql_has('INNER JOIN positions ON(employees.id = positions.employee_id)',
	...
	public function test_gh93_and_gh100_eager_loading_respects_association_options() {
		Venue::$has_many = array(array('events', 'class_name' => 'Event', 'order' => 'id asc',
												 'conditions' => array('length(title) = ?', 14)));
		$venues = Venue::find(array(2, 6), array('include' => 'events'));

		$this->assert_sql_has("WHERE length(title) = ? AND venue_id IN(?,?) ORDER BY id asc",
	...
	public function test_gh_23_relationships_with_joins_to_same_table_should_alias_table_name() {
		...
		$select =
			"books.*, authors.name as to_author_name, {$c->quote_name('from_')}.name as from_author_name, {$c->quote_name('another')}.name as another_author_name";
